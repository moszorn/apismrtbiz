...
migrate-up: $(MIGRATE) ## Apply all (or N up) migrations.
	@ read -p "How many migration you wants to perform (default value: [all]): " N; \
 	migrate  -database $(MYSQL_DSN) -path=database/migrations up ${NN}

 .PHONY: migrate-down
migrate-down: $(MIGRATE) ## Apply all (or N down) migrations.
	@read -p "How many imgration you wants to perform (default value: [all]): " N; \
	migrate  -database $(MYSQL_DSN) -path=database/migrations down ${NN}
